...
async def play(_, message: Message):
    ...
    lel = await message.reply("🔎 **العثور علي  💫 الاغنيه ❤️ ❰ المحبوبه 🚬 ❱...**")
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                try:
                    ...
                    await lel.edit(
                        "<b>قم بترقيتي كمسؤول عن المجموعة 💫</b>")
                    return

                try:
                    await USER.join_chat(invitelink)
                    await USER.send_message(
                        message.chat.id, "** انضم الحساب المساعد إلى  المجموعة لتشغيل الموسيقى 🎶**")

                except UserAlreadyParticipant:
                    pass
                except Exception:
                    await lel.edit(
                        f"<b>❰خطأ انتظار ❱</b>\n  مرحبًا  تعذر على الحساب المساعد الانضم المجموعة ) </i>")
    ...
    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            raise DurationLimitError(
                f"**❰ ° الأغنية  🎸 ° ❱   الأطول من دقيقة  {DURATION_LIMIT} غير مسموح بتشغيل  ▶ ❤️🥀**"
            )
        ...
        if (dur / 60) > DURATION_LIMIT:
            await lel.edit(
                f"**❰ ° الاغنية 🎸 ° ❱ اطول من  {DURATION_LIMIT} 𝐌𝐢𝐧𝐮𝐭𝐞'𝐒 𝐀𝐫𝐞𝐧'𝐭 𝐀𝐥𝐥𝐨𝐰𝐞𝐝 𝐓𝐨 𝐏𝐥𝐚𝐲 ▶ ❤️🥀**"
            )
            return
        requested_by = message.from_user.first_name
        await generate_cover(requested_by, title, views, duration, thumbnail)
